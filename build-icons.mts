...
const inputDir = path.join(cwd, 'public', 'icons', 'sources')
...
async function generateIconFiles() {
    ...
    const versionedSpriteFile = `sprite.${timestamp}.svg`
    const spriteFilepath = path.join(outputDir, versionedSpriteFile)
    ...
    envContent = envContent.replace(/^NEXT_PUBLIC_SPRITE_PATH=.*/m, '') + `NEXT_PUBLIC_SPRITE_PATH=${versionedSpriteFile}`

    fs.writeFileSync(envPath, envContent.trim(), 'utf8')
    console.log(`Generated ${files.length} icons. New version saved as '${versionedSpriteFile}'.`)
...
async function removeOldSpriteFiles() {
    const oldSprites = glob.sync(path.join(outputDir, 'sprite.*.svg'))
    for (const oldSprite of oldSprites) {
        await fsExtra.remove(oldSprite)
        console.log(`Deleted old sprite version: ${path.basename(oldSprite)}`)
    }
}
